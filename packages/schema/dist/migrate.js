...
exports.migratePageGraph = migratePageGraph;
/**
 ...
 */
function migratePageGraph(data, fromVersion, toVersion) {
    ...
    if (isNaN(from) || isNaN(to)) {
        throw new Error(`migratePageGraph: non-numeric versions are not supported ` +
            `(fromVersion="${fromVersion}", toVersion="${toVersion}")`);
    }
    if (from > to) {
        throw new Error(`migratePageGraph: downgrade from version ${from} to ${to} is not supported`);
    }
    let current = data;
    for (let v = from; v < to; v++) {
        const key = `${v}->${v + 1}`;
        const fn = exports.migrations.get(key);
        if (!fn) {
            throw new Error(`migratePageGraph: no migration registered for step "${key}". ` +
